...
const moveToArchive = async () => {
    ...
    try {
        const response = await axios.put(
            `${BASE_API_URL}/api/employees/${props.employee._id}/archive`,
            {},
            {
                headers: {
        ...
        if (response.status === 200) {
            ...
            statusMessage.value = 'Employee moved to archive successfully';
            setTimeout(() => {
                statusMessage.value = '';
                emit('close');
            }, 3000);
        ...
        statusMessage.value = 'Failed to move employee to archive';
        setTimeout(() => (statusMessage.value = ''), 3000);
    } finally {
...
<template>
    <Modal :show="show" @close="$emit('close')" max-width="md" maxHeight="80vh">
        <div class="p-4 border-b border-gray-300">
            <h2 class="text-lg font-semibold">Confirm Move to Archive</h2>
        </div>

        <div class="p-4">
            <p>Move {{ employee.firstName }} {{ employee.lastName }} to archive?</p>
        </div>
