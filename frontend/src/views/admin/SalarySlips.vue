...
export default {
    ...
    methods: {
            ...
            try {
                ...
                const response = await axios.get(`${BASE_API_URL}/api/config`, {
                    headers: {
                        ...
                        'user-role': 'admin',
                        'user-id': this.authStore.admin?._id || '',
                    },
                });
                this.config = {
        ...
        async fetchEmployees() {
            ...
            this.statusMessage = '';
            const token = this.authStore.accessToken;
            const userId = this.authStore.admin?._id || '';
            if (!token) throw new Error('No authentication token found');
            try {
        ...
        async showPayslipHistory(employee) {
            ...
            if (!token) {
                ...
                this.$router.push('/admin-login'); // Assuming Vue Router is used
                return;
            }
            ...
            try {
                ...
                if (response.status === 201 || response.status === 200) {
                    ...
                    this.showSuccessMessage(
                        `Payslip generated for ${employee.name} - ${payslip.paydayType === 'mid-month' ? 'Mid-Month' : 'End-of-Month'
                        }!`
                    );
                }
            } catch (error) {
...
async generateAllPayslips() {
        ...
        async generatePayslipNow(employee) {
            ...
            try {
                ...
                if (response.status === 201 || response.status === 200) {
                    ...
                    this.showSuccessMessage(
                        `Payslip generated now for ${employee.name} - ${payslipData.paydayType === 'mid-month' ? 'Mid-Month' : 'End-of-Month'
                        }!`
                    );
                }
            } catch (error) {
        ...
        async downloadPayslip() {
            ...
            try {
                ...
                link.href = url;
                link.download = `Payslip_${this.selectedEmployee.name}_${this.selectedPayslip.paydayType === 'mid-month' ? 'Mid-Month' : 'End-of-Month'
                    }.pdf`;
                                    ...
                                    <tr>
                                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 cursor-pointer"
                                            ...
                                            Pay Date <span class="material-icons text-xs">{{ sortPreviousField ===
                                                'payDate' ? (sortPreviousAsc ? 'arrow_upward' : 'arrow_downward') : ''
                                                }}</span>
                                        </th>
                                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 cursor-pointer"
                                            @click="sortPreviousPayslips('position')">
                                            Position <span class="material-icons text-xs">{{ sortPreviousField ===
                                                'position' ? (sortPreviousAsc ? 'arrow_upward' : 'arrow_downward') : ''
                                                }}</span>
                                        </th>
                                        ...
                                        <tr>
                                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 cursor-pointer"
                                                ...
                                                Pay Date <span class="material-icons text-xs">{{ sortNewField ===
                                                    'payDate' ? (sortNewAsc ? 'arrow_upward' : 'arrow_downward') : ''
                                                    }}</span>
                                            </th>
                                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 cursor-pointer"
                                                @click="sortNewPayslips('position')">
                                                Position <span class="material-icons text-xs">{{ sortNewField ===
                                                    'position' ? (sortNewAsc ? 'arrow_upward' : 'arrow_downward') : ''
                                                    }}</span>
                                            </th>
