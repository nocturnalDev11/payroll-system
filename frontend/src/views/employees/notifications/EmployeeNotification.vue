            ...
            <header class="bg-white rounded-xl shadow-lg p-6 mb-6 border border-gray-100">
                <div class="flex justify-between items-center">
                    <h1 class="text-2xl font-bold text-gray-900 flex items-center gap-2">
                        <span class="material-icons text-teal-600">notifications</span>
                        Notifications
                    </h1>
                    <button @click="markAllAsRead"
                        class="text-sm font-medium text-teal-600 hover:text-teal-800 transition-colors disabled:opacity-50"
                        :disabled="unreadCount === 0 || isLoadingAction" aria-label="Mark all notifications as read">
                        <span v-if="isLoadingAction"
                            class="inline-block h-4 w-4 border-2 border-teal-600 border-t-transparent rounded-full animate-spin mr-2"></span>
                        Mark all as read
                    </button>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-lg border border-gray-100">
                <div v-if="isLoading" class="p-10 flex justify-center items-center">
                    <div class="flex flex-col items-center">
                        <div
                            class="loader ease-linear rounded-full border-4 border-t-4 border-teal-200 h-12 w-12 mb-4 animate-spin">
                        </div>
                ...
                <div v-else-if="notifications.length === 0" class="p-12 text-center">
                    <div class="rounded-full bg-teal-100 p-4 mb-4 inline-flex">
                        <span class="material-icons text-teal-600 text-3xl">notifications_off</span>
                    </div>
                    ...
                    <div v-for="notification in notifications" :key="notification._id"
                        ...
                        <div class="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button v-if="notification.status === 'Unread'" @click="markAsRead(notification._id)"
                                title="Mark as read"
                                class="flex items-center text-teal-500 hover:text-teal-700 p-1 rounded-full hover:bg-teal-100 transition-colors"
                            ...
                            <button @click="deleteNotification(notification._id)" title="Delete notification"
                                class="flex items-center text-red-500 hover:text-red-700 p-1 rounded-full hover:bg-red-100 transition-colors"
